...
*/
    ...
    var config = {
        ...
        // 默认菜单配置
        menus: ['head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'textIndent','textLineHeight','justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo'],
...
// 原型
    TextIndent.prototype = {
        ...
        onClick: function onClick(e) {
            // 点击菜单将触发这里
            // editor.cmd.do('textIndent');
            var editor = this.editor;
            var $selectionElem = editor.selection.getSelectionContainerElem();
            if($selectionElem[0].style.textIndent){
                $selectionElem[0].style.textIndent='';
            }else{
                $selectionElem[0].style.textIndent='2em';
            }
        },
        // 试图改变 active 状态
        tryChangeActive: function tryChangeActive(e) {
...
*/
// 构造函数
    function textLineHeight(editor) {
        var _this = this;
        ...
        this.droplist = new DropList(this, {
            ...
            $title: $('<p>行高</p>'),
            type: 'list', // droplist 以列表形式展示
            list: [{ $elem: $('<span><i class="fa fa-text-height"></i> 1.0</span>'), value: 'lineHeight1' }, { $elem: $('<span><i class="fa fa-text-height"></i> 1.5</span>'), value: 'lineHeight2' }, { $elem: $('<span><i class="fa fa-text-height"></i> 2.0</span>'), value: 'lineHeight3' }],
            onClick: function onClick(value) {
                var editor = _this.editor;
                var $selectionElem = editor.selection.getSelectionContainerElem();
                if(value=='lineHeight1'){
                    $selectionElem[0].style.lineHeight="1.0"
                }
                if(value=='lineHeight2'){
                    $selectionElem[0].style.lineHeight="1.5"
                }
                if(value=='lineHeight3'){
                    $selectionElem[0].style.lineHeight="2.0"
                }
            }
        });
    }
...
// 原型
    textLineHeight.prototype = {
        constructor: textLineHeight,
...
// 构造函数
    function Justify(editor) {
        ...
        this.droplist = new DropList(this, {
            ...
            $title: $('<p>对齐方式</p>'),
            type: 'list', // droplist 以列表形式展示
            list: [{ $elem: $('<span><i class="w-e-icon-paragraph-left"></i> 靠左</span>'), value: 'justifyLeft' }, { $elem: $('<span><i class="w-e-icon-paragraph-center"></i> 居中</span>'), value: 'justifyCenter' }, { $elem: $('<span><i class="w-e-icon-paragraph-right"></i> 靠右</span>'), value: 'justifyRight' },{ $elem: $('<span><i class="fa fa-align-justify"></i> 两端</span>'), value: 'justifyFull' }],
            onClick: function onClick(value) {
                // 注意 this 是指向当前的 List 对象
                _this._command(value);
            }
...
// 存储菜单的构造函数
    ...
    MenuConstructors.textIndent = TextIndent;

    MenuConstructors.textLineHeight = textLineHeight;
    ...
    UploadImg.prototype = {
        ...
        uploadImg: function uploadImg(files) {
            ...
            arrForEach(resultFiles, function (file) {
                ...
                formdata.append('maxSize', 500*1024*1024);
            });
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if (!hooks.customInsert && result.errno != '0') {
                            ...
                            if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                                ...
                                editor.cmd.do('insertHTML', '<video src="' + result.data + '" style="width:100%" controls autobuffer />');
